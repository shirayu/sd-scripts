...
def convert(args):
    ...
    ), f"v1 or v2 is required to load checkpoint / checkpointの読み込みにはv1/v2指定が必要です"
    # assert (
    ...
    msg = (
        "checkpoint"
        if is_load_ckpt
        else ("Diffusers" + (" as fp16" if args.fp16 else ""))
    )
    ...
    if is_load_ckpt:
        ...
        (
            ...
            v2_model,
            args.model_to_load,
            unet_use_linear_projection_in_v2=args.unet_use_linear_projection,
        )
    else:
    ...
    # 変換して保存する
    msg = (
        ("checkpoint" + ("" if save_dtype is None else f" in {save_dtype}"))
        if is_save_ckpt
        else "Diffusers"
    )
