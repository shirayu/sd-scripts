...
def process(args):
    assert (
        not args.resize_fit
    ) or args.resize_face_size is None, f"resize_fit and resize_face_size can't be specified both / resize_fitとresize_face_sizeはどちらか片方しか指定できません"
    assert (
        args.crop_ratio is None or args.resize_face_size is None
    ), f"crop_ratio指定時はresize_face_sizeは指定できません"
    ...
    if args.crop_size is None:
        ...
        assert (
            len(tokens) == 2
        ), f"crop_size must be 'width,height' / crop_sizeは'幅,高さ'で指定してください"
    ...
    if args.crop_ratio is None:
        ...
        assert (
            len(tokens) == 2
        ), f"crop_ratio must be 'horizontal,vertical' / crop_ratioは'幅,高さ'の倍率で指定してください"
    ...
    paths = (
        glob.glob(os.path.join(args.src_dir, "*.png"))
        + glob.glob(os.path.join(args.src_dir, "*.jpg"))
        ...
        for i, face in enumerate(faces):
            ...
            if crop_width is not None or crop_h_ratio is not None:
                ...
                if crop_h_ratio is not None:
                    cur_crop_width = int(face_size * crop_h_ratio + 0.5)
                    cur_crop_height = int(face_size * crop_v_ratio + 0.5)
                ...
                if args.resize_face_size is not None:
                    ...
                    if w < cur_crop_width:
                        print(
                            f"image width too small/ 画像の幅がcrop sizeより小さいので画質が劣化します: {path}"
                        )
                        scale = cur_crop_width / w
                    if h < cur_crop_height:
                        print(
                            f"image height too small/ 画像の高さがcrop sizeより小さいので画質が劣化します: {path}"
                        )
                        scale = cur_crop_height / h
                    if args.resize_fit:
            ...
            with open(
                os.path.join(
                    args.dst_dir,
                    f"{basename}{face_suffix}_{cx:04d}_{cy:04d}_{fw:04d}_{fh:04d}{output_extension}",
                ),
                "wb",
            ) as f:
                buf.tofile(f)
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--src_dir", type=str, help="directory to load images / 画像を読み込むディレクトリ"
    )
    parser.add_argument(
        "--dst_dir", type=str, help="directory to save images / 画像を保存するディレクトリ"
    )
    parser.add_argument(
        "--rotate",
        action="store_true",
        help="rotate images to align faces / 顔が正立するように画像を回転する",
    )
    parser.add_argument(
        "--resize_fit",
        action="store_true",
    ...
    parser.add_argument(
        "--debug", action="store_true", help="render rect for face / 処理後画像の顔位置に矩形を描画します"
    )
