...
def train(args):
    ...
    if init_token_ids is not None:
        for i, token_id in enumerate(token_ids_XTI):
            token_embeds[token_id] = token_embeds[
                init_token_ids[(i // 16) % len(init_token_ids)]
            ]
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                encoder_hidden_states = torch.stack(
                    [
                        train_util.get_hidden_states(
                            args, s, tokenizer, text_encoder, weight_dtype
                        )
    ...
    updated_embs = (
        text_encoder.get_input_embeddings().weight[token_ids_XTI].data.detach().clone()
    )
...
def save_weights(file, updated_embs, save_dtype):
    ...
    for i, layer_name in enumerate(XTI_layers):
        state_dict[layer_name] = (
            updated_embs[i].squeeze(0).detach().clone().to("cpu").to(save_dtype)
        )
