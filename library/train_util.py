...
V2_STABLE_DIFFUSION_PATH = (
    "stabilityai/stable-diffusion-2"  # ここからtokenizerだけ使う v2とv2.1はtokenizer仕様は同じ
)
...
class ImageInfo:
    def __init__(
        ...
        self.latents_original_size: Tuple[
            int, int
...
class BaseSubset:
    def __init__(
        ...
        self.token_warmup_step = (
            token_warmup_step  # N（N<1ならN*max_train_steps）ステップ目でタグの数が最大になる
        )
                ...
                if (
                    ...
                    tokens_len = (
                        math.floor(
                            (self.current_step)
                            * (
                                (len(tokens) - subset.token_warmup_min)
                                / (subset.token_warmup_step)
                            )
                        )
    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if image_info.text_encoder_outputs1 is not None:
                ...
                (
                    text_encoder_outputs1,
                    text_encoder_outputs2,
                    text_encoder_pool2,
    ...
    def __init__(
        ...
        for subset in subsets:
            ...
            extra_imgs.extend(
                [
                    cond_img_path
                    for cond_img_path in conditioning_img_paths
                    if cond_img_path not in cond_imgs_with_img
                ]
            )
...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument(
        "--max_train_steps", type=int, default=1600, help="training steps / 学習ステップ数"
    )
    parser.add_argument(
        "--max_train_epochs",
        type=int,
    ...
    parser.add_argument(
        "--seed", type=int, default=None, help="random seed for training / 学習時の乱数のseed"
    )
    parser.add_argument(
...
def transform_if_model_is_DDP(text_encoder, unet, network=None):
    ...
    return (
        model.module if type(model) == DDP else model
        for model in [text_encoder, unet, network]
...
def get_noise_noisy_latents_and_timesteps(args, noise_scheduler, latents):
    ...
    if args.ip_noise_gamma:
        noisy_latents = noise_scheduler.add_noise(
            latents, noise + args.ip_noise_gamma * torch.randn_like(latents), timesteps
        )
    else:
