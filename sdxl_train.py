...
def get_block_params_to_optimize(
    unet: SdxlUNet2DConditionModel, block_lrs: List[float]
) -> List[dict]:
...
def train(args):
    ...
    if block_lrs is None:
        ...
        params_to_optimize = get_block_params_to_optimize(
            training_models[0], block_lrs
        )  # U-Net
        for m in training_models[1:]:  # Text Encoders if exists
            params_to_optimize.append(
                {"params": m.parameters(), "lr": args.learning_rate}
            )
